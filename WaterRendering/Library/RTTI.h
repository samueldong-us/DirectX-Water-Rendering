...
#define RTTI_DECLARATIONS(Type, ParentType)\
	public:\
		...
		static const unsigned int& TypeIdClass() { return runtimeTypeId; }\
		virtual Library::RTTI* QueryInterface(const unsigned int id) const\
		{\
			if (id == runtimeTypeId) { return (RTTI*)this; }\
			else { return Parent::QueryInterface(id); }\
		}\
		virtual bool Is(const unsigned int id) const\
		{\
			if (id == runtimeTypeId) { return true; }\
	...
	private:\
		static unsigned int runtimeTypeId;

#define RTTI_DEFINITIONS(Type) unsigned int Type::runtimeTypeId = (unsigned int)&Type::runtimeTypeId;
}